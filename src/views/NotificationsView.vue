...
import type { NotificationDto, PagedResponse, NotificationType } from '@/api/types';
...
const fetchNotifications = async () => {
  ...
  try {
    ...
    const response = await httpClient<PagedResponse<NotificationDto>>(`/me/notifications?${params.toString()}`);
...
const markAsRead = async (id: string) => {
  ...
  try {
    await httpClient(`/me/notifications/${id}/read`, { method: 'PATCH' });
...
const deleteNotification = async (id: string) => {
  ...
  actionLoadingId.value = id;
  try {
    await httpClient(`/me/notifications/${id}`, { method: 'DELETE' });
...
const markAllRead = async () => {
  try {
    await httpClient('/me/notifications/read-all', { method: 'POST' });
...
<template>
  <div class="space-y-6 max-w-4xl mx-auto pb-10">
    <div class="flex items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-black tracking-tight">{{ t('notifications.title') }}</h1>
        <p class="text-muted-foreground text-sm">{{ t('dashboard.overview') }}</p>
      </div>
      <Button variant="outline" size="sm" @click="markAllRead" :disabled="isLoading">
        <Check class="mr-2 size-4" /> {{ t('common.markAllRead') }}
      </Button>
    </div>
          ...
          <CardContent class="p-4 flex gap-4 items-start">
            ...
            <div class="flex flex-col gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button v-if="!notif.IsRead" variant="ghost" size="icon" class="size-8" @click="markAsRead(notif.Id)" :disabled="actionLoadingId === notif.Id">
                ...
                <div v-else class="size-4 border-2 border-t-foreground border-transparent rounded-full animate-spin" />
              </Button>
    ...
    <!-- Pagination (Referencing AdminUsersSection Style) -->
