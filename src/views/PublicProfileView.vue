...
import {
  ...
  Image as ImageIcon, GraduationCap, Key, Mail, Download, Copy, Check, User, FileText, Clock
...
<template>
  <div class="w-full pb-20">
    ...
    <!-- Loading State -->
    <div v-if="isLoading" class="container max-w-7xl mx-auto px-4 py-10 space-y-8">
          ...
          <div class="lg:col-span-8 xl:col-span-9 pt-8 min-w-0">
            <Tabs defaultValue="overview" class="w-full">
              <TabsList class="w-full justify-start h-auto p-0 bg-transparent border-b border-border rounded-none gap-6 mb-8 overflow-x-auto no-scrollbar">
                <TabsTrigger value="overview" class="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent px-0 py-3 font-bold text-muted-foreground data-[state=active]:text-foreground transition-all"><BookOpen class="size-4 mr-2" /> {{ t('publicProfile.about') }}</TabsTrigger>
                <TabsTrigger value="projects" class="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent px-0 py-3 font-bold text-muted-foreground data-[state=active]:text-foreground transition-all"><FolderGit2 class="size-4 mr-2" /> {{ t('publicProfile.projects') }} <Badge variant="secondary" class="ml-2">{{ projects.length }}</Badge></TabsTrigger>
                <TabsTrigger v-if="isOrg" value="members" class="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent px-0 py-3 font-bold text-muted-foreground data-[state=active]:text-foreground transition-all"><Users class="size-4 mr-2" /> {{ t('publicProfile.members') }} <Badge variant="secondary" class="ml-2">{{ members.length }}</Badge></TabsTrigger>
                <TabsTrigger v-if="isPersonal" value="experience" class="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent px-0 py-3 font-bold text-muted-foreground data-[state=active]:text-foreground transition-all"><Briefcase class="size-4 mr-2" /> {{ t('publicProfile.experience') }}</TabsTrigger>
                <TabsTrigger value="resources" class="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent px-0 py-3 font-bold text-muted-foreground data-[state=active]:text-foreground transition-all"><ImageIcon class="size-4 mr-2" /> {{ t('publicProfile.resources') }}</TabsTrigger>
              </TabsList>
                  ...
                  <div class="flex flex-col gap-4">
                    <Card v-for="cert in certificates" :key="cert.Id" class="border-border/60">
                      <CardContent class="p-5 space-y-4">
                        <div class="flex flex-col md:flex-row justify-between gap-4">
                          <div class="space-y-2 flex-1 min-w-0">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm text-muted-foreground">
                              <div v-if="cert.Email" class="flex items-center gap-2"><Mail class="size-3.5" /> {{ cert.Email }}</div>
                              <div v-if="cert.CreatedAt" class="flex items-center gap-2"><Clock class="size-3.5" /> {{ t('resources.createdAt') }}: {{ formatDate(cert.CreatedAt) }}</div>
                              <div v-if="cert.ExpiresAt" class="flex items-center gap-2"><Calendar class="size-3.5" /> {{ t('resources.expiresAt') }}: {{ formatDate(cert.ExpiresAt) }}</div>
                            </div>
                          </div>
                          <div class="flex flex-wrap gap-2 shrink-0">
                            <Button v-if="cert.Content" variant="outline" size="sm" @click="downloadCert(cert.Content, cert.Name)"><Download class="size-4 mr-2" /> {{ t('common.download') }}</Button>
                            <Button v-if="cert.Content" variant="secondary" size="sm" @click="copyToClipboard(cert.Content, cert.Id + 'content')">
                        ...
                        <div class="space-y-2">
                          <div class="text-xs font-bold uppercase text-muted-foreground flex items-center gap-2"><FileText class="size-3" /> {{ t('dashboard.fingerprint') }}</div>
                        ...
                        <div v-if="cert.Content" class="space-y-2">
                          <div class="text-xs font-bold uppercase text-muted-foreground">{{ t('resources.content') }}</div>
                          <pre class="text-[10px] font-mono p-4 bg-muted/30 rounded-lg border overflow-x-auto max-h-40 select-all whitespace-pre-wrap break-all">{{ cert.Content }}</pre>
                        </div>
                      </CardContent>
                    </Card>
                  </div>
